...
תיקייה זו מרכזת **תיאור מערכת** (ארכיטקטורה, זרימות נתונים, שכבות אחריות). להנחיות קצרות באנגלית למראיינים (הרצה, משקולות, דמו) — ראו [`README.md`](../README.md) בשורש המאגר. מדריך עומק בעברית: [`PROJECT_GUIDE_HE.md`](PROJECT_GUIDE_HE.md).
